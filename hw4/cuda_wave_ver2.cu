...
// implement algorithm described in: Exact and Approximated error of the FMA
// by Sylvie Boldo and Jean-Michel Muller
__device__ float Fast2Sum(float a, float b, float *err) {
  if (abs(a) < abs(b)) { float t=a; a=b; b=t; }
  float x = a + b;
  float t = x - a;
  *err = b - t;
  return x;
...
__device__ float Fast2Mult(float a, float b, float *err) {
  float x = a * b;
  *err = __fmaf_rn(a, b, -x);
  return x;
}
...
__global__ void updateKernel(float *gpuMem, int nsteps, int tpoints) {
  ...
  int i;
  for (i = 1; i <= nsteps; i++) {
    float newval, e1, e2, e3;
    float moment = Fast2Sum(2.0f * value, -oldval, &e1);
    float damp = Fast2Mult((-2.0f) * sqtau, value, &e2);
    newval = Fast2Sum(moment, damp, &e3);
    newval = e1 + e2 + e3 + newval;
    oldval = value;
    value = newval;
  }
